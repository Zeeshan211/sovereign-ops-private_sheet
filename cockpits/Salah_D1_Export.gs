...
const SALAH_D1_EXPORT_VERSION = 'Salah_D1_Export v0.1.0';
...
function buildSalahD1ExportSql() {
  ...
  _salahD1Alert(
    ...
    'Next: copy SQL from D1_Salah_Export and paste into Cloudflare D1 after review.'
  );
}
...
function verifySalahD1ExportSource() {
  ...
  const values = salah.getRange(SALAH_D1_GRID_START_ROW, 1, SALAH_D1_GRID_DAYS, 11).getValues();
...
function appendSalahD1ExportMenu() {
  try {
    ...
    Logger.log('Salah D1 export menu failed: ' + e);
  }
}

function _salahD1ReadRows_(salah) {
  const values = salah.getRange(SALAH_D1_GRID_START_ROW, 1, SALAH_D1_GRID_DAYS, 11).getValues();
  const displayValues = salah.getRange(SALAH_D1_GRID_START_ROW, 1, SALAH_D1_GRID_DAYS, 11).getDisplayValues();
  const rows = [];

  for (let i = 0; i < SALAH_D1_GRID_DAYS; i++) {
    const sourceRow = SALAH_D1_GRID_START_ROW + i;
    const row = values[i];
    const displayRow = displayValues[i];

    const day = _salahD1DateToIso_(row[0]);
    if (!day) continue;

    const dayOfMonth = Number(Utilities.formatDate(row[0], SALAH_D1_TZ, 'd'));
    ...
    const counts = _salahD1DailyCounts_(prayerEntries);
...
function _salahD1BuildPrayerEntry_(prayerName, rawCode, normalizedCode, day, sourceRow, sourceColumn) {
  const scoreValue = _salahD1ScoreValue_(normalizedCode);
  const id = ['salah', day, prayerName].join('_');
...
function _salahD1ReadPrayerTimes_(salah, now) {
  const row3 = salah.getRange(3, 2, 1, 5).getDisplayValues()[0];
  const times = row3.map(function(cell) {
    return _salahD1ExtractTime_(cell);
  });

  const today = Utilities.formatDate(now, SALAH_D1_TZ, 'yyyy-MM-dd');
...
function _salahD1BatchInsertSql_(batch) {
  return 'INSERT OR REPLACE INTO salah_export_batches (' +
    'id, source_system, source_tab, source_version, source_layout, export_type, exported_at, rows_read, rows_written, status, notes, created_at' +
    ') VALUES (' +
...
function _salahD1DailyInsertSql_(row, exportBatchId, exportedAt, sourceLayout) {
  return 'INSERT OR REPLACE INTO salah_daily_status (' +
    ...
    'source_system, source_tab, source_version, source_layout, source_row, source_checksum, export_batch_id, exported_at, updated_at' +
    ') VALUES (' +
...
function _salahD1PrayerEntryInsertSql_(entry, row, exportBatchId, exportedAt) {
  return 'INSERT OR REPLACE INTO salah_prayer_entries (' +
    ...
    'source_system, source_tab, source_version, source_row, source_column, source_checksum, export_batch_id, exported_at, updated_at' +
    ') VALUES (' +
...
function _salahD1PrayerTimesInsertSql_(times) {
  return 'INSERT OR REPLACE INTO salah_prayer_times (' +
    'day, city, country, timezone, method, fajr_time, dhuhr_time, asr_time, maghrib_time, isha_time, provider, provider_date, fetched_at, is_fallback, source_note, updated_at' +
    ') VALUES (' +
...
function _salahD1RecoveryInsertSql_(entry, row, exportBatchId) {
  const id = ['salah_recovery', row.day, entry.prayer_name].join('_');

  return 'INSERT OR REPLACE INTO salah_recovery_items (' +
    'id, day, prayer_name, source_entry_id, raw_code, recovery_status, recovery_due_date, recovered_at, reason, recovery_note, source_system, export_batch_id, updated_at' +
    ') VALUES (' +
    _salahD1Sql_(id) + ', ' +
    _salahD1Sql_(row.day) + ', ' +
    _salahD1Sql_(entry.prayer_name) + ', ' +
...
function _salahD1WriteExportSheet_(ss, sql, meta) {
  let out = ss.getSheetByName(SALAH_D1_EXPORT_TAB);
  ...
  out.getRange(6, 1).setValue('Rows Written');
  out.getRange(6, 2).setValue(meta.rowsWritten);

  out.getRange(8, 1).setValue('Copy SQL below into Cloudflare D1 Console after review');
  out.getRange(9, 1).setValue(sql.join('\n'));

  out.setColumnWidth(1, 1200);
  out.setColumnWidth(2, 260);
  out.getRange(1, 1, 8, 2).setFontWeight('bold');
  out.getRange(9, 1).setWrap(true).setFontFamily('Roboto Mono').setFontSize(9);
}
...
function _salahD1DetectLayout_(salah) {
  const lastCol = salah.getLastColumn();
